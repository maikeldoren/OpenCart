...
#### Changes in release 13.5.0
  ...
  + ** Admin & Checkout Improvements**
    ...
    + Auto-Correct Pending Status: Automatically selects "Mollie Pending" status, preventing alphabetical OpenCart defaults.
    + Clean UI Rendering: JavaScript sanitizer strips broken HTML tags from bank icons in hidden input fields.
    + Smart UI Toggle: "Notify Customer" checkbox auto-checks when enabling "Send Mollie Payment Link".
    + Payment Method Parsing: Replaced `explode()` with `str_starts_with` to prevent crashes on specific methods.
    + Order Info Stability: Verified existence of `_embedded->refunds` before iterating.
    + Stock Synchronization: Refactored stock mutation logic to be DRY and safe.
